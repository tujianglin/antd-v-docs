...
import type { CascaderProps, FormItemProps, FormProps } from 'antd-v';
import { AutoComplete, Button, Cascader, Checkbox, Col, Form, Input, InputNumber, Row, Select } from 'antd-v';
...
export default defineComponent({
  setup() {
    ...
    const onWebsiteChange = (value: string) => {
      autoCompleteResult.value = value ? ['.com', '.org', '.net'].map((domain) => `${value}${domain}`) : [];
    };

    const websiteOptions = computed(() =>
      autoCompleteResult.value.map((website) => ({
        label: website,
        value: website,
      })),
    );
    return () => (
      ...
      >
        ...
        <Form.Item name="website" label="Website" rules={[{ required: true, message: 'Please input website!' }]}>
          <AutoComplete options={websiteOptions.value} onChange={onWebsiteChange} placeholder="website">
            <Input />
          </AutoComplete>
        </Form.Item>
